...
const CatalogMenu = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <>
                ...
                {menu.products.map((product) => (
                  <Link key={product.name} to={product.link} className="catalog-menu-product">
                    <Typography
                      ...
                      component="div"
                      color="textSecondary"
                      sx={{ "&:hover": { color: "rgb(89, 89, 89)", cursor: "pointer", textDecoration: "underline" } }}
                    >
                      {product.name}
                    </Typography>
                  </Link>
      ...
      <div className={`catalog-menu-overlay ${showMenu ? "show" : ""}`} onClick={onClose}></div>
